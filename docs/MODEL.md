...
- l'échec est **routé vers le formulaire** (`form.engineErrors`, invariant 38),
  jamais transformé en refus métier ni écrit dans la console.
...
| 36 | Où vont les erreurs asynchrones du moteur | **routées vers le formulaire** — un tampon `FormController.engineErrors` que le consommateur **lit** (**hors du snapshot**, sink de validation injecté **au site du catch** via `ValidationContext.reportFailure`), taguées `hook-error` ou `guard-violation` — jamais journalisées | un hook qui rejette, une règle qui casse, une garde violée n'ont personne à qui remonter : la promesse qui les porte n'est attendue par personne. La console est un cul-de-sac non testable et invisible en production. Le canal est **hors du snapshot** parce que `FormView` n'a délibérément pas de souscription (invariant 10), et jamais sur `FieldView`. Le sink est injecté au catch plutôt que porté par `publish`/`ValidatorState`/`equals`, qui n'ont pas à connaître le diagnostic (invariant 26 : un sink de diagnostic n'est pas une mutation d'état). Le discriminant vient d'une `EngineGuardError` typée que les gardes lèvent, classée par `instanceof` : le consommateur sépare un bug de sa logique (`hook-error`) d'un mauvais usage de l'API (`guard-violation`). Le contexte détaché ne porte pas le sink — silence assumé. La surface est **pull**, pas **push** : un accesseur `engineErrors`, aucun abonnement `onEngineError`. Une erreur du moteur est un crash déjà rattrapé (invariant 35) — un enregistrement de diagnostic, pas un événement à traiter en temps réel ; le consommateur le lit quand il a une raison (après un `submit()` refusé, dans un panneau de debug). Un canal d'abonnement rouvrirait une seconde mécanique de souscription à côté de `listen`, par formulaire, pour quelque chose de rare et déjà encaissé. Si un besoin de télémétrie réel apparaît, ce sera un **sink optionnel injecté à la construction** — pas un registre d'abonnés runtime |
...
| 38 | Le moteur ne loggue jamais | aucun `console.*` dans `packages/form/src` : une erreur de hook — rejet asynchrone, garde du moteur violée, règle qui casse — est **routée** vers le formulaire, taguée `hook-error` ou `guard-violation`. `FieldController` la remet au `FormController` par `host.reportEngineError`, qui la bufferise (borné, vidé au `reset()`) pour lecture par `form.engineErrors` — un tampon, pas un canal d'abonnement (arbitrage 36). La validation route par un sink injecté au site du catch (`ValidationContext.reportFailure`), propagé base → composite → différé ; le contexte détaché ne le porte pas |
