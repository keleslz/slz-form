...
| 38 | Le moteur ne loggue jamais | aucun `console.*` dans `packages/form/src` : une erreur de hook — rejet asynchrone, garde du moteur violée, règle qui casse — est **routée** vers le formulaire, taguée `hook-error` ou `guard-violation`. `FieldController` la remet au `FormController` par `host.reportEngineError`, qui la bufferise (borné, vidé au `reset()`) et notifie `onEngineError`. La validation route par un sink injecté au site du catch (`ValidationContext.reportFailure`), propagé base → composite → différé ; le contexte détaché ne le porte pas |
