# slz-lib-v5 — modélisation du core
...
> Statut : implémenté et vérifié dans le navigateur (`src/page/showcase`).
...
`src/page/showcase` monte **tous les types de champ dans une seule vue** :
...
5. **Sort de l'existant.** `src/slz-lib` et `src/slz-lib-2` sont toujours là et
   portent leurs 43 erreurs TypeScript d'origine. À supprimer quand ce modèle
   est validé.
